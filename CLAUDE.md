...
`cmd/grove` is the cobra CLI, one file per command. Beyond `exec`, `ls`, `hold`, `release`, `forget`, and the `start`/`stop`/`restart` trio, that is `install`/`uninstall` (the privileged and trust-store path), `doctor` (a finding per check, with a remedy where there is one to name, and usually where a platform change needs its matching edit), `init`, `env`, and `context`. `internal/*` holds everything with logic in it:
...
| `shell` | text a shell will run: quoting values, the argv steps a privileged plan is made of, and naming grove itself |
...
**Ports are derived, then remembered.** An FNV hash of slug plus entry name lands inside 20000-20999, below Linux's ephemeral range, and collisions walk to the next free port. Every *detached* port is written down, in `ports.json` beside the CA: without that record colliding entries re-resolve in whatever order they ask next restart, and an entry can be handed the port another worktree's stack is listening on. A database URL pointing at someone else's data is why the file exists. Recording only walked ports left the winner of a collision unnamed once its daemon was gone, so after a reboot the loser could take the port its stack was still on. A walk leaves a port another entry's record names to that entry, listening or not, so a stopped stack keeps its port. Reclaiming a quiet one needs the range to fill, which a thousand ports against a few dozen entries means never, so a record outlives its worktree, which is what hands the ports back if the checkout is remade under the same name. `grove forget` is what drops one, and doctor names the contexts whose worktree is gone; the record carries the path it was written for so both can tell.

**Environment layering has a deliberate order** (`layer` in `cmd/grove/exec.go`): trust variables, then `env_files` (which yield to anything already in the caller's environment), then `GROVE_CONTEXT` and friends, then grove's resolved values, then `.env.local`. Grove's own win over the caller's shell, because a route naming a port it did not lease would be a lie. `.env.local` is the one tier above them, and it yields to nothing: if it yielded to the shell while grove's values did not, exporting a name would hand it back to grove and the file's line would silently stop applying. It cannot set `GROVE_CONTEXT`, `GROVE_PORT`, `GROVE_HOST` or `GROVE_URL`, and a project that lists `.env.local` in its own `env_files` keeps it in that lower tier.
